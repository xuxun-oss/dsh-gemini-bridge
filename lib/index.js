// dsh-vision-imagen — 宿主半（Host）。
// 为 DeepSeek 模型桥接「视觉 + 生图」能力：注册 vision_read / generate_image /
// edit_image 三个工具，注入工具引导，并提供配置读写、模型列表与图片服务的
// HTTP 路由供浏览器设置页调用。
//
...
// 并提供可点击的图片链接（/api/vision-imagen/images/<file>）。
...
export const name = 'dsh-vision-imagen';
...
const IMAGE_DIR = join(process.env.DSH_HOME || join(homedir(), '.dsh'), 'vision-imagen-images');

// ---- 配置持久化（~/.dsh/vision-imagen.json，跨会话/工作区共享）----
function cfgPath() {
  return join(process.env.DSH_HOME || join(homedir(), '.dsh'), 'vision-imagen.json');
}
function loadCfg() {
  try {
...
function saveImageFile(buf, mime, nameBase) {
  ...
  return { path: abs, name, url: '/api/vision-imagen/images/' + name };
}
...
const NO_KEY = 'Vision Imagen 未配置 API Key，请在 设置 → Vision Imagen 中填写。';
...
function createApiHandler() {
  return async (req, res) => {
    ...
    const path = url.pathname.replace(/^\/api\/vision-imagen/, '');
    try {
...
export function apply(ctx) {
  ...
  ctx.systemPrompt.section({
    ...
    order: 150,
    text: () => (cfgNow().apiKey
      ? '【Vision Imagen】本会话已启用图像能力桥接（后端: ' + cfgNow().provider + '）。\n'
        + '- 需要识别、读取、描述、分析图片内容时，调用 vision_read（image 传本地路径或 http(s) 图片 URL）。\n'
        + '- 用户要求生成/绘制图片时，调用 generate_image（prompt 写详细描述；生成后会自动用后端视觉模型检查并反馈）。\n'
        + '- 用户要求修改、调整、优化一张已有图片时，调用 edit_image。\n'
        + '- 生成的图片会保存到 ~/.dsh/vision-imagen-images/ 并在结果中提供图片链接。'
      : ''),
  });
